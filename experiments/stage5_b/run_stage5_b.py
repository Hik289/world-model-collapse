#!/usr/bin/env python3
"""Stage 5 B Full Grid — gpt-4o-mini × stateful_puzzle × 4×4 grid × n=50.

Director Path (Stage 5 B Full Grid): cross-model G1 replication on mini.
Pre-registered: STAGE_4_PREREQUISITE_CHECKLIST.md Item #9.

Reads:
  - experiments/stage5_b/stage5_b_task_seeds.json (800 cells)

Writes:
  - data/raw_logs/stage5_b_step.jsonl
  - data/raw_logs/stage5_b_episode.jsonl
  - data/raw_logs/cost_tracker.jsonl (incremental)
  - experiments/stage5_b/stage5_b_results.json (on completion)
  - experiments/stage5_b/completed_cells.json (every 50 ep, atomic)
...
RESULTS_PATH = OUT_DIR / "stage5_b_results.json"
...
def save_completed(task_ids: set[str]) -> None:
    payload = {
        "schema_version": "stage5_b_completed_v1",
        "updated_jst": jst_now(),
...
def analyze_stage5(outcomes: list[EpisodeOutcome]) -> dict:
    ...
    by_env_sc_dd = defaultdict(lambda: {"n": 0, "n_success": 0, "n_error": 0,
    ...
    for o in outcomes:
        ...
        d = by_env_sc_dd[f"{env}|sc={sc},dd={dd}"]
        d["n"] += 1
    ...
    out = {"per_cell": {}}
    for key, d in by_env_sc_dd.items():
        n = d["n"]
        out["per_cell"][key] = {
            "n": n,
...
def main() -> int:
    ...
    ap.add_argument("--cost-cap", type=float, default=300.0)
    args = ap.parse_args()

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    LOG_DIR.mkdir(parents=True, exist_ok=True)

    print(f"[stage5b] === Stage 5 B Full Grid G1-trigger dispatch (mini cross-model) ===")
    print(f"[stage5b] start: {jst_now()}  cost_cap=${args.cost_cap:.2f}")
    ...
    if completed:
        ...
        print(f"[stage5b] resuming: {len(completed)} done previously, {len(cells)} remaining")
    else:
        cells = all_cells
        print(f"[stage5b] fresh start: {len(cells)} cells")
    ...
    def progress(i, n, o: EpisodeOutcome):
        ...
        with lock:
            ...
            print(f"[stage5b {i}/{n}] {o.cell.task_id} {success}{tag} "
            ...
            if i % args.checkpoint_every == 0 or i == n:
                ...
                print(f"[stage5b]   checkpoint: {len(state['completed'])}/{len(all_cells)} cells done; cum_cost=${cum:.2f}")
            # Hard cost cap
    ...
    print(f"[stage5b] launching run_pilot_slice (bounded-wave, n_workers={args.n_workers}) ...")
    outs = run_pilot_slice(
        cells=cells,
        client=client,
        step_jsonl_path=LOG_DIR / "stage5_b_step.jsonl",
        episode_jsonl_path=LOG_DIR / "stage5_b_episode.jsonl",
        cost_tracker=ct,
